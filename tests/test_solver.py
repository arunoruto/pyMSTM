"""Tests for pymstm.MSTM/MstmResult -- the unified bindings/CLI
interface used for benchmarking and cross-checking the two backends."""

import pydantic
import pytest

from pymstm import MSTM, MstmNotFoundError, find_mstm_binary
...
def test_mismatched_positions_length_raises():
    with pytest.raises(pydantic.ValidationError, match="positions"):
        MSTM(radii=[5.0, 3.0], positions=[(0, 0, 0)], ref_re=[1.5], ref_im=[0.0])
...
def test_cli_mueller_zero_incidence_ok():
    ...
    MSTM(**_SINGLE_SPHERE, cli=True, compute_mueller=True, incident_beta_deg=0.0)
...
def test_solve_bindings_single_sphere():
    result = MSTM(**_SINGLE_SPHERE).solve()
...
def test_solve_cli_single_sphere():
    result = MSTM(**_SINGLE_SPHERE, cli=True).solve()
...
def test_bindings_and_cli_mueller_agree_on_same_grid():
    ...
    problem = MSTM(**_TWO_SPHERE, compute_mueller=True, n_theta=21)
    r_bindings = problem.solve()
    r_cli = problem.model_copy(update={"cli": True}).solve()
